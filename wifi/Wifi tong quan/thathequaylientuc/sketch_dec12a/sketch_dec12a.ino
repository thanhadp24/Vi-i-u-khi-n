...
int remainingSteps = 0;    // Số bước còn lại để quay
bool nguocchieu = false;   // Hướng quay (true = ngược chiều kim đồng hồ)
bool isMotorRunning = false; // Trạng thái động cơ (đang quay hoặc không)
int step = 0;
...
void motorControl() {
  ...
  if (steps > 0) {
    ...
    server.send(200, "text/plain", "Động cơ quay " + chieu + " chiều kim đồng hồ " + String(degrees) + " độ");
  } else {
    server.send(400, "text/plain", "Số độ không hợp lệ");
  }
}
...
void setup() {
  myStepper.setSpeed(15); // Tốc độ động cơ
  Serial.begin(9600);
...
void loop() {
  server.handleClient();
  MDNS.update();
  // Kiểm tra nếu động cơ đang hoạt động
  if (isMotorRunning && remainingSteps > 0) {
    myStepper.step(nguocchieu ? -1 : 1); // Quay từng bước
    remainingSteps--;                   // Giảm số bước còn lại
    // Nếu đã hoàn thành lượt quay
    if (remainingSteps == 0 && step != 0) {
      remainingSteps = step;
      delay(1000); // Dừng 1 giây trước khi chờ lệnh mới
    }
  }
  if (step == 0) {
